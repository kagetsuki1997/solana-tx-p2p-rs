...
type WrapResult<T> = std::result::Result<T, Box<dyn std::error::Error + Send + Sync + 'static>>;
...
enum EventType {
    ...
    Input(String),
    MessageTrigger,
    Swarm(SwarmEvent<RecipeBehaviorEvent>),
}

#[derive(NetworkBehaviour)]
#[behaviour(to_swarm = "RecipeBehaviorEvent")]
...
#[derive(Debug)]
enum RecipeBehaviorEvent {
    Floobsub(FloodsubEvent),
    Mdns(MdnsEvent),
}

impl From<FloodsubEvent> for RecipeBehaviorEvent {
    fn from(event: FloodsubEvent) -> Self { Self::Floobsub(event) }
}

impl From<MdnsEvent> for RecipeBehaviorEvent {
...
async fn create_new_recipe(name: &str, ingredients: &str, instructions: &str) -> WrapResult<()> {
...
async fn publish_recipe(id: usize) -> WrapResult<()> {
    let mut local_recipes = read_local_recipes().await?;
...
async fn read_local_recipes() -> WrapResult<Recipes> {
    let content = fs::read(STORAGE_FILE_PATH).await?;
    let result = serde_json::from_slice(&content)?;
    Ok(result)
}

async fn write_local_recipes(recipes: &Recipes) -> WrapResult<()> {
...
#[tokio::main]
async fn main() -> WrapResult<()> {
    pretty_env_logger::init();

    let message_duration = random::<u8>() % 10 + 5;
    let message_topic = Topic::new("message");
    let mut message_timer = time::interval(Duration::from_secs(message_duration.into()));

    info!("Peer Id: {}, duration: {message_duration}", PEER_ID.clone());
    ...
    loop {
        let evt = {
            tokio::select! {
                _ = message_timer.tick() => Some(EventType::MessageTrigger),
                line = stdin.next_line() => Some(EventType::Input(line.expect("can get line").expect("can read line from stdin"))),
                response = response_rcv.recv() => Some(EventType::Response(response.expect("response exists"))),
                event = swarm.select_next_some() => {
                    Some(EventType::Swarm(event))
                },
            }
        };
        ...
        if let Some(event) = evt {
            match event {
                ...
                EventType::MessageTrigger => {
                    warn!("Message trigger");
                    let message = format!("Message from {}", PEER_ID.clone());
                    swarm.behaviour_mut().floodsub.publish(message_topic.clone(), message);
                }
                EventType::Swarm(swarm_event) => match swarm_event {
                    SwarmEvent::Behaviour(RecipeBehaviorEvent::Floobsub(
                        ...
                        if let Ok(resp) = serde_json::from_slice::<ListResponse>(&msg.data) {
                            ...
                            warn!("Receive message from {:?}: {message}", msg.source);
                        }
                    }
                    SwarmEvent::Behaviour(RecipeBehaviorEvent::Mdns(mdns_event)) => {
                        match mdns_event {
                    ...
                    _ => {
                        info!("Unhandled event {swarm_event:?}")
                    }
                },
            }
        }
...
async fn handle_list_recipes(cmd: &str, swarm: &mut Swarm<RecipeBehaviour>) {
    ...
    match rest {
        ...
        Some(recipes_peer_id) => {
            let req = ListRequest { mode: ListMode::One(recipes_peer_id.to_owned()) };
            let json = serde_json::to_string(&req).expect("can jsonify request");
            swarm.behaviour_mut().floodsub.publish(TOPIC.clone(), json);
        }
        None => {
